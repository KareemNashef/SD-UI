...
class ComfyProgressService {
  ...
  final ValueNotifier<GenerationProgress> notifier =
      ValueNotifier(GenerationProgress.idleFor(BackendKind.comfy));

  GenerationProgress? get current => notifier.value;
  ...
  /// Opens (or reuses) the WebSocket for [profile]. Safe to call
  ...
  Future<void> connect(ServerProfile profile, {bool force = false}) async {
    if (_disposed) return;
    if (!force &&
        _socket != null &&
    ...
    try {
      final uri = profile.wsUri('/ws', {'clientId': ensureClientId()});
  ...
  void nudgeRunningIfQueued() {
    if (_state != GenerationState.queued) return;
    _state = GenerationState.running;
    _emit();
  }
  ...
  void _emit({String? jobId}) {
    notifier.value = GenerationProgress(
      backend: BackendKind.comfy,
      state: _state,
      jobId: jobId ?? _trackedPromptId,
  ...
  void _scheduleReconnect() {
    _socket = null;
    _subscription = null;
    if (_disposed || _connectedProfile == null) return;
    ...
    _reconnectTimer = Timer(delay, () {
      ...
      if (profile != null && !_disposed) connect(profile);
    });
  }
